...
void checkIfIsNotInBlackList(char* ip_address) {
	FILE *black_list;
	black_list = fopen("black_list.txt", "w");
	fclose(black_list);
}
...
int main() {
    ...
    while(1) {
		...
		char ip_address[INET_ADDRSTRLEN], port[BUFFER_SIZE], client_information_message[BUFFER_SIZE] = "INFO: Server connected with: ";
		inet_ntop( AF_INET, &client_address, ip_address, INET_ADDRSTRLEN );
        	printf("INFO: Server connected with: %s:%d\n", ip_address, ntohs(client_address.sin_port));

		sprintf(port, "%d", ntohs(client_address.sin_port));
		strncat(client_information_message, ip_address, strlen(ip_address));
		strncat(client_information_message, ":", 1);
		strncat(client_information_message, port, strlen(port));
		strncat(client_information_message, "\n", 2);
		printMessageToLogFile(client_information_message);

		checkIfIsNotInBlackList(ip_address);
