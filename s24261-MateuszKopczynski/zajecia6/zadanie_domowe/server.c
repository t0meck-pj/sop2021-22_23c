...
void createLog(char* log, char* userLog);
...
//        createLog("[-] Error on listening \n");
    ...
    while (1) {
        ...
        char client_port_string[30];
        int client_port_int = ntohs(client_addr.sin_port);

        sprintf(client_port_string, "%d", client_port_int);
        stract(ip, client_port_string);
        ...
        if (newsockfd > 0) {
            ...
            createLog("[+] Client connected (IP: %s, PORT: %d) \n", ip);
        } else {
